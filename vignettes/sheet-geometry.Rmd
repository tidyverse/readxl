...
The extent of the data rectangle can be determined in various ways:
  ...
  * **Mixed**: In typical use, `read_excel()`'s geometry arguments often imply that certain limits are **discovered**, others are **bounded**, and another might be **set**. This will be more clear in the concrete examples below.
...
Summary: `skip` tells `read_excel()` to skip *at least this many* spreadsheet rows before looking for any data.
...
`n_max` is an upper bound. It will never cause empty rows to be included in the tibble. Note how we get 3 data rows here, even though `n_max` is much more.
...
Illustrated with the `deaths.xlsx` example sheet that features junk rows both before and after the data rectangle. By specifying the data rectangle precisely, we get to enjoy the benefit of `read_excel()`'s column type guessing.

```{r}
read_excel(readxl_example("deaths.xlsx"), range = "arts!A5:F15")
```

Illustrated with the `geometry.xlsx` and a rectangle that only partially overlaps the data. Note the use of default column names and the leading column of `NA`s.
