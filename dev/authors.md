# Authors and Citation

## Authors

- **[Hadley Wickham](https://hadley.nz)**. Author.
  [](https://orcid.org/0000-0003-4757-117X)

- **[Jennifer Bryan](https://jennybryan.org)**. Author, maintainer.
  [](https://orcid.org/0000-0002-6983-2759)

- **[![Posit](https://www.tidyverse.org/posit-logo.svg)](https://www.posit.co)**.
  Copyright holder, funder.  
  Copyright holder of all R code and all C/C++ code without explicit
  copyright attribution

- **Marcin Kalicinski**. Contributor, copyright holder.  
  Author of included RapidXML code

- **Komarov Valery**. Contributor, copyright holder.  
  Author of included libxls code

- **Christophe Leitienne**. Contributor, copyright holder.  
  Author of included libxls code

- **Bob Colbert**. Contributor, copyright holder.  
  Author of included libxls code

- **David Hoerl**. Contributor, copyright holder.  
  Author of included libxls code

- **Evan Miller**. Contributor, copyright holder.  
  Author of included libxls code

## Citation

Source:
[`DESCRIPTION`](https://github.com/tidyverse/readxl/blob/main/DESCRIPTION)

Wickham H, Bryan J (2026). *readxl: Read Excel Files*. R package version
1.5.0.9000, <https://readxl.tidyverse.org>.

    @Manual{,
      title = {readxl: Read Excel Files},
      author = {Hadley Wickham and Jennifer Bryan},
      year = {2026},
      note = {R package version 1.5.0.9000},
      url = {https://readxl.tidyverse.org},
    }
